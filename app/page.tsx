...
export default function Home() {
  ...
  const [bannerOne, setBannerOne] = useState<HomepageBanner | null>(null);
  const [weeklyDealIds, setWeeklyDealIds] = useState<string[]>([]);
  const [bannerTwo, setBannerTwo] = useState<HomepageBanner | null>(null);
  ...
  useEffect(() => {
    const catalogRequest = categoryApi
      .list()
      .then((categoryResult) => {
    ...
    void Promise.allSettled([catalogRequest, homepageRequest]).then(() =>
      setPageReady(true),
    );
  }, []);
  useEffect(() => {
  ...
  if (!pageReady) return <StorefrontLoader />;
