...
export default function ProductManager() {
    ...
    const load = useCallback(async () => { setLoading(true); try { const result = await catalogApi.products(search, brand, active, page, size, category, "", { minimumRating: rating ? Number(rating) : undefined, minimumPrice: minPrice, maximumPrice: maxPrice, sort, stockStatus: stock, hasImage: imageStatus }); setProducts(result.items); setTotal(result.total); setPages(result.pages); if (page > result.pages) setPage(result.pages) } catch (e) { setError(e instanceof Error ? e.message : "Could not load products") } finally { setLoading(false) } }, [search, brand, active, page, size, category, stock, imageStatus, minPrice, maxPrice, rating, sort]);
