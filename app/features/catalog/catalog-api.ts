...
export const catalogApi = {
  ...
  products: (search = "", brand = "", status = "", page = 1, size = 20, categoryL1 = "", categoryL2 = "", filters: { brands?: string[]; minimumRating?: number; minimumPrice?: string; maximumPrice?: string; sort?: string; stockStatus?: string; hasImage?: string } = {}) => call<{items: Product[]; total: number; page:number; size:number; pages:number}>(`/catalog/products/list?${new URLSearchParams({ page:String(page), size:String(size), ...(search && { search }), ...(brand && { brand }), ...(status && { is_active: status }), ...(categoryL1 && { category_l1: categoryL1 }), ...(categoryL2 && { category_l2: categoryL2 }), ...(filters.brands?.length && { brands: filters.brands.join("|") }), ...(filters.minimumRating && { min_rating: String(filters.minimumRating) }), ...(filters.minimumPrice && { min_price: filters.minimumPrice }), ...(filters.maximumPrice && { max_price: filters.maximumPrice }), ...(filters.sort && { sort: filters.sort }), ...(filters.stockStatus && { stock_status: filters.stockStatus }), ...(filters.hasImage && { has_image: filters.hasImage }) })}`),
