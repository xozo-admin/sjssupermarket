...
function ProductSelectionPanel({ title, description, selected, saving, hasChanges, onToggle, onSave }: { title: string; description: string; selected: string[]; saving: boolean; hasChanges: boolean; onToggle: (id: string) => void; onSave: () => void }) {
  ...
  return <section className="fresh-picks-admin">
    <div className="fresh-picks-selected"><div><h2>{title}</h2><p>{description}</p></div><button className="primary-button" disabled={saving || !hasChanges} onClick={onSave}>{saving ? "Saving..." : "Save Products"}</button></div>
...
export default function HeroManager() {
  ...
  useEffect(() => {
    ...
    void Promise.all([categoryApi.list(), homepageApi.topCategories()])
      .then(([categoryResult, selected]) => {
        setCategories(categoryResult.items);
        setSelectedCategories(selected);
        setOriginalCategories(selected);
      })
      .catch((reason) =>
        setError(reason instanceof Error ? reason.message : "Could not load top categories")
      );
    void homepageApi.freshPicks().then((items) => {
    ...
    }).catch((reason) => setError(reason instanceof Error ? reason.message : "Could not load trending products"));
    void homepageApi.weeklyDeals().then((items) => {
      setSelectedWeeklyDeals(items);
      setOriginalWeeklyDeals(items);
    }).catch((reason) => setError(reason instanceof Error ? reason.message : "Could not load weekly deals"));
  }, []);
  ...
  const saveCategories = async () => {
    ...
    try {
      const result = await homepageApi.updateTopCategories(selectedCategories);
  ...
  const saveProducts = async () => {
    ...
    setError("");
    try {
      const result = await homepageApi.updateFreshPicks(selectedProducts);
  ...
  const saveTrending = async () => {
    setSavingTrending(true); setError(""); try {
      const result = await homepageApi.updateTrendingProducts(selectedTrending);

      setSelectedTrending(result);
      setOriginalTrending(result);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not save trending products"); } finally { setSavingTrending(false); }
  };
  ...
  const saveWeeklyDeals = async () => {
    setSavingWeeklyDeals(true); setError(""); try {
      const result = await homepageApi.updateWeeklyDeals(selectedWeeklyDeals);
      setSelectedWeeklyDeals(result);
      setOriginalWeeklyDeals(result);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not save weekly deals"); } finally { setSavingWeeklyDeals(false); }
  };
...
const weeklyDealsChanged =
  ...
  return <div className="homepage-admin">
    ...
    <aside className="homepage-steps">
