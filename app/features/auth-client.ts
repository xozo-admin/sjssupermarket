...
export function logout(){const session=getAuthSession();if(session?.refresh_token)void fetch(`${API}/auth/logout`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:session.refresh_token})});localStorage.removeItem(KEY);sessionStorage.removeItem(KEY);window.dispatchEvent(new Event("sjs-auth-updated"))}
